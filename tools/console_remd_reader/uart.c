 ...
 */
int uart_start(struct uart_device* dev, bool canonical) {
	struct termios *tty;
	int fd;
	int rc;
...
int uart_readn(struct uart_device* dev, char *buf, size_t buf_len) {
	...
	FD_SET(dev->fd, &fds);	/* add file descriptor to the set */

	timeout.tv_sec = 0;
	timeout.tv_usec = 1000000 * sec;

	rc = select(dev->fd + 1, &fds, NULL, NULL, &timeout);
	if (rc < 0) {
		if (!uart_sigint_fired)
			printf("%s: failed to check uart descriptor: %d\n", __func__, rc);
		return rc;
	}
	if (rc == 0) {
		printf("%s: no data available within %d sec\n", __func__, sec);
		return 0;
	}

	rc = read(dev->fd, buf, buf_len);
	if (rc < 0) {
		printf("%s: failed to read uart data: %d\n", __func__, rc);
	}
...
int uart_reads(struct uart_device* dev, char *buf, size_t buf_len) {
	...
	rc = uart_readn(dev, buf, buf_len - 1);
	if (rc < 0)
		return rc;
