...
#define DD_SDCS			DD_SS
#define BV_SDCS			BV_SS

#define SDCS_LO()		REG_SPI &= ~BV_SDCS		/* Set MMC_CS = low */
#define	SDCS_HI()		REG_SPI |= BV_SDCS		/* Set MMC_CS = high */
...
#define	FCLK_SLOW()		spi_set_clk(clock_div_128)		/* Set SPI clock for initialization (100-400kHz) */
#define	FCLK_FAST()		spi_set_clk(clock_div_2)		/* Set SPI clock for read/write (20MHz max) */
